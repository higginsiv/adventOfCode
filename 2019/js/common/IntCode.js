...
async function run(memory, pointer = 0n, input, out = [], relative = 0n, eventEmitter) {
  ...
  while (opCode !== 99) {
    ...
    let parameterModes = opCodeWhole
      .substring(0, opCodeWhole.length - 2)
      .split('')
      .reverse()
      .map((x) => {
        if (x === '') {
    ...
    switch (opCode) {
      ...
      case OP_5:
        pointerMod = BigInt(
          jumpIfTrue(pointer + 1n, pointer + 2n, memory, pointer, parameterModes, relative),
        );
        break;
      case OP_6:
        pointerMod = BigInt(
          jumpIfFalse(pointer + 1n, pointer + 2n, memory, pointer, parameterModes, relative),
        );
        break;
      case OP_7:
