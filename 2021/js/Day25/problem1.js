...
while (moved) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < data[i].length; j++) {
      let cuke = data[i][j];
      let canMove =
        (j === data[i].length - 1 && lastCanMove) ||
        (j < data[i].length - 1 && data[i][j + 1] === EMPTY);
  ...
  for (let i = 0; i < data[0].length; i++) {
    ...
    for (let j = 0; j < data.length; j++) {
      let cuke = data[j][i];
      let canMove =
        (j === data.length - 1 && lastCanMove) || (j < data.length - 1 && data[j + 1][i] === EMPTY);
