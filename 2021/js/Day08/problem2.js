        ...
        while (!solved) {
            digits.forEach((digit) => {
                if (!solvedDigits[1] && digit.length === 2) {
                    ...
                    if (
                        JSON.stringify(solvedDigits[9]) != JSON.stringify(temp) &&
                        JSON.stringify(solvedDigits[0]) != JSON.stringify(temp)
                    ) {
                    ...
                    if (
                        JSON.stringify(solvedDigits[0]) != tempString &&
                        ...
                        outputEntries.forEach((x) => {
                            ...
                            solvedDigits.forEach((y, index) => {
                                let solvedDigit = y;
                                if (
                                    digits.every((x) => solvedDigit.includes(x)) &&
                                    digits.length === solvedDigit.length
                                ) {
                                    outputString += index;
                                }
                            });
