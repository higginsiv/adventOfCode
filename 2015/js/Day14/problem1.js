...
fr.getInput(year,day).forEach(x => {
    ...
    reindeer.push({fly: parseInt(SPEED), time: parseInt(TIME), rest: parseInt(REST)});
});

let answer = reindeer.reduce((best, curr) => {
    let distance = curr.fly * (curr.time * Math.floor(SECONDS / (curr.time + curr.rest)) + (curr.time > (SECONDS % (curr.time + curr.rest)) ? (SECONDS % (curr.time + curr.rest)) : curr.time));
