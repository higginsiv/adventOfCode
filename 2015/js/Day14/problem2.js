...
fr.getInput(year,day).forEach(x => {
    ...
    reindeer.push({name: NAME, fly: parseInt(SPEED), time: parseInt(TIME), rest: parseInt(REST)});
    points.set(NAME, 0);
});

for (let i = 1; i <= SECONDS; i++) {
    let inLead = reindeer.reduce((best, curr) => {
        let distance = curr.fly * (curr.time * Math.floor(i / (curr.time + curr.rest)) + (curr.time > (i % (curr.time + curr.rest)) ? (i % (curr.time + curr.rest)) : curr.time));
