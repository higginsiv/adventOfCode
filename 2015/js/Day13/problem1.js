...
let answer = perms.reduce((total, curr, index) => {
    ...
    for (let i = 0; i < curr.length; i++) {
        ...
        let partnerAHappiness = people.get(curr[i]).get(curr[partnerAIndex]) == null ? 0 : people.get(curr[i]).get(curr[partnerAIndex]);
        let partnerBHappiness = people.get(curr[i]).get(curr[partnerBIndex]) == null ? 0 : people.get(curr[i]).get(curr[partnerBIndex]);
