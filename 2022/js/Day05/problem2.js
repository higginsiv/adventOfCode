...
movData.forEach((instruction) => {
  ...
  crateData[to - 1].push(
    ...crateData[from - 1].slice(crateData[from - 1].length - amount)
  );
  crateData[from - 1] = crateData[from - 1].slice(
    0,
    crateData[from - 1].length - amount
  );
});

let answer = crateData.reduce(
  (crateTops, currentCol) => (crateTops += currentCol.pop()),
  ""
);
