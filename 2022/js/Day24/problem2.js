...
function findGoal(row, col, turns, goalRow, goalCol) {
    ...
    if (point.minStepsToReach.get(state) == null || point.minStepsToReach.get(state) > turns) {
        ...
        if (!point.isGoal && turns < points[goalRow][goalCol].globalMin) {
            ...
            isValidMove(row + 1, col, turns) && queue.push(new Iteration(row + 1, col, turns, goalRow, goalCol));
            isValidMove(row - 1, col, turns) && queue.push(new Iteration(row - 1, col, turns, goalRow, goalCol));
            isValidMove(row, col + 1, turns) && queue.push(new Iteration(row, col + 1, turns, goalRow, goalCol));
            isValidMove(row, col - 1, turns) && queue.push(new Iteration(row, col - 1, turns, goalRow, goalCol));
            isValidMove(row, col, turns) && queue.push(new Iteration(row, col, turns, goalRow, goalCol));
        }
    }
}

function isValidMove(newRow, newCol, turns) {
    let rowInBounds = newRow >= 0 && newRow < points.length && data[newRow][newCol] !== VALLEY_WALL;
...
let wayThereAgain = points[END_ROW][END_COL].globalMin;

console.log('Year ' + year + ' Day ' + day + ' Puzzle ' + part + ': ' + wayThereAgain);