...
const DATA = fr.getInput(YEAR,DAY).map((line, index) => {
    let matches = line.match(/(\d+)/g).map(x => parseInt(x));
    return [{x: matches[0], y: matches[1], z: matches[2]}, {x: matches[3], y: matches[4], z: matches[5]}, index].toSorted((a,b) => a.z - b.z);
