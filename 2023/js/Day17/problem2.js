...
while (queue.length > 0) {
    ...
    neighbors.forEach((neighbor) => {
        ...
        if (current.steps < 4 && direction !== current.direction && current.direction !== -1) {
            return;
        }
        ...
        if (
            steps <= 10 &&
            direction !== getOppositeDirection(current.direction)
        ) {
            insertIntoSortedQueue(
                queue,
                new State(x, y, direction, steps, heatLoss)
            );
            visited.add(key);
        }
    });
