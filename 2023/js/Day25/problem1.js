...
const DATA = fr.getInput(YEAR,DAY).map(line => line.match(/\w+/g)).forEach(line => {
    let mainNode = line[0];
...
for (let i = 0; i < 3; i++) {
    ...
    let [node1, node2] = [...betweenness.keys()].sort((a, b) => betweenness.get(b) - betweenness.get(a))[0].split('-');
