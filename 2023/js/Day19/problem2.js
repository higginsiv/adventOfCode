...
DATA[0].split(EOL).forEach((line) => {
    ...
    rules = rules.map((rule, index) => {
        ...
        let [_, category, symbol, number] =
            evaluation.match(/(\w+)([<>])(\d+)/);
...
let answer = validRanges.reduce((lineTotal, lineCurr) => {
        ...
        return lineTotal + (lineCurr[0][1] - lineCurr[0][0] + 1) * (lineCurr[1][1] - lineCurr[1][0] + 1) * (lineCurr[2][1] - lineCurr[2][0] + 1) * (lineCurr[3][1] - lineCurr[3][0] + 1);
}, 0);
