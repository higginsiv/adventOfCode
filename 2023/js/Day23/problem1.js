...
while (queue.length > 0) {
    ...
    neighbors.forEach(neighbor => {
        ...
        queue.push({point: neighbor, distance: current.distance + 1, visited: new Set(current.visited)});
    });
}
...
function getValidNeighbors(point, value) {
    ...
    let includeEast = point.y < GRID[0].length - 1 && value !== NORTH && value !== WEST && value !== SOUTH;
    let includeSouth = point.x < GRID.length - 1 && value !== EAST && value !== WEST && value !== NORTH;
