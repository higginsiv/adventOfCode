    ...
    for (let j = i + 1; j < junctionPoints.length; j++) {
        ...
        while (queue.length > 0) {
            ...
            if (current.point.x === junctionPoints[j].x && current.point.y === junctionPoints[j].y) {
                ...
                junctionToJunctionDistances.get(junctionPoints[i].key).set(junctionPoints[j].key, current.distance);
                ...
                junctionToJunctionDistances.get(junctionPoints[j].key).set(junctionPoints[i].key, current.distance);
                break;
            }
            ...
            neighbors.forEach(neighbor => {
                ...
                if (junctionPointsString.has(neighborKey) && neighborKey !== generateKey(junctionPoints[j].x, junctionPoints[j].y)) {
                    return;
                }
                ...
                queue.push({point: neighbor, distance: current.distance + 1, visited: newVisited});
            });
...
while (queue.length > 0) {
    ...
    let neighbors = [...junctionToJunctionDistances.get(current.key).keys()].filter(key => !current.visited.has(key));
    ...
    neighbors.forEach(neighbor => {
        ...
        distancesToJunctions.set(neighbor, current.distance + junctionToJunctionDistances.get(current.key).get(neighbor));
        queue.push({key: neighbor, distance: current.distance + junctionToJunctionDistances.get(current.key).get(neighbor), visited: newVisited});
    });
}
...
function sumRemainingEdges(junctionToJunctionDistances, visited) {
    ...
    for (let i = 0; i < junctionPoints.length; i++) {
        ...
        for (let j = 0; j < junctionPoints.length; j++) {
            ...
            max = Math.max(max, junctionToJunctionDistances.get(junctionPoints[i].key).get(junctionPoints[j].key));
        }
        sum += max;
    }
    return sum;
