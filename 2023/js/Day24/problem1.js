...
let lines = [];
const DATA = fr.getInput(YEAR,DAY).map(x => x.match(/-?\d+/g).map(x => parseInt(x))).forEach(line => {
    ...
    lines.push({m: m, b:b, startX: x, xDir: vx > 0 ? 1 : -1, yDir: vy > 0 ? 1 : -1});
});
...
function doLinesIntersectInRanges(line1, line2, rangeStart, rangeEnd) {
    ...
    let doLinesIntersectInRange = (x >= rangeStart && x <= rangeEnd && y >= rangeStart && y <= rangeEnd);
    ...
    return doLinesIntersectInRange && isIntersectionInFutureForLine1 && isIntersectionInFutureForLine2 ? 1 : 0;
}
