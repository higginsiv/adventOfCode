...
const DATA = fr.getInput(YEAR,DAY).forEach(line => {
    ...
    switch (command) {
        case 'rect':
            ...
            break;
        case 'rotate':
            let [direction, index, amount] = [args[0], args[1].split('=').map(Number)[1], parseInt(args[3])];
            switch (direction) {
                case 'row':
                    screen[index] = screen[index].slice(-amount).concat(screen[index].slice(0, -amount));
                    break;
                case 'column':
...
let answer = screen.reduce((total, row) => total + row.reduce((total, pixel) => total + pixel, 0), 0);
