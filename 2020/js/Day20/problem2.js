...
for (let i = 0; i < gridWidth; i++) {
    for (let j = 0; j < gridWidth; j++) {
        ...
        if (j === 0) {
            ...
            tileToPlace = findAndOrientTileToPlace(
                edgeToMatch,
                northMatchesSouthernEdge
            );
        } else {
...
let totalWaterPoints = bigArt.reduce((total, line) => {
    return total + line.reduce((lineTotal, point) => {
...
console.log(
    'Year ' + year + ' Day ' + day + ' Puzzle ' + part + ': ' + answer
);
console.timeEnd();