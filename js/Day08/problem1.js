		...
		digits.forEach(digit => {
			if (!solvedDigits[1] && digit.length === 2) {
				...
				outputDigits.forEach(x => {
					let digits = x.split('');
					if (digits.every(x => solvedDigits[1].includes(x)) && x.length === solvedDigits[1].length
						|| digits.every(x => solvedDigits[4].includes(x)) && x.length === solvedDigits[4].length
						|| digits.every(x => solvedDigits[7].includes(x)) && x.length === solvedDigits[7].length
						|| digits.every(x => solvedDigits[8].includes(x)) && x.length === solvedDigits[8].length
					) {
						numSuccess++;
					}
				});
